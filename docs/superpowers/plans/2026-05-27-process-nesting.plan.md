# Process Nesting Templates 实现计划

> **面向自动化工作者：** 必需子技能： 使用 superpowers:subagent-driven-development (推荐) 或 superpowers:executing-plans 按任务逐步实现此计划. 步骤使用复选框 (`- [ ]`) 语法进行跟踪.

**目标：** Create reusable template files that encode the six-layer (Superpower + TaskMaster + Speckit + DDD + BMAD + QA) constitution-centric workflow, so any new project can drop them in and start.

**架构：** Eight template files under `docs/superpowers/templates/`. Each is a standalone, copy-paste-ready artifact. Templates are project-agnostic —— no hardcoded tech stack, no svg-project specifics. The BMAD agents are rewritten from the AWS/Serverless originals into generic full-stack agents. The constitution template follows the five-paragraph structure from the spec.

**技术栈：** Templates are markdown, bash, and YAML. No code compilation needed.

---

### 任务 1： Constitution Template

**涉及文件：**
- 新建：`docs/superpowers/templates/constitution-template.md`

- [ ] **步骤 1：create constitution template file**
...
- Spec → TaskMaster task breakdown → Speckit clarify → Speckit plan → TDD implement → QA 关卡
...
- [ ] **步骤 2：verify template placeholders are discoverable**

Run: `grep -n '<!-- fill' docs/superpowers/templates/constitution-template.md`
预期结果： 8 lines with `<!-- fill -->` markers that new projects can search-replace.
...
### 任务 2： BMAD Dev Agent (Pirlo) Template

**涉及文件：**
- 新建：`docs/superpowers/templates/bmad-dev-template.md`

- [ ] **步骤 1：create Pirlo agent template**
...
<persona>
  ...
  <principles>
    ...
    - 提交代码 per bounded context: small, reviewable commits
...
- [ ] **步骤 2：verify no AWS/Serverless/Cognito references leak through**
...
### 任务 3： BMAD QA Agent (Quinn) Template

**涉及文件：**
- 新建：`docs/superpowers/templates/bmad-qa-template.md`

- [ ] **步骤 1：create Quinn agent template**
...
<activation critical="MANDATORY">
  ...
  <step n="7">提交代码 tests with message: "test: add tests for <bounded-context>"</step>
...
### 任务 4： BMAD SM Agent (Bob) Template

**涉及文件：**
- 新建：`docs/superpowers/templates/bmad-sm-template.md`

- [ ] **步骤 1：create Bob agent template**
...
### 任务 5： QA Quality Gate Script Template

**涉及文件：**
- 新建：`docs/superpowers/templates/quality-gate.sh`

- [ ] **步骤 1：create quality-gate.sh template**
...
### 任务 6： DDD Directory Scaffold Script

**涉及文件：**
- 新建：`docs/superpowers/templates/ddd-scaffold.sh`

- [ ] **步骤 1：create scaffold script**
...
echo "  2. Quinn writes tests → Pirlo implements → QA 关卡"
...
### 任务 7： CLAUDE.md Template

**涉及文件：**
- 新建：`docs/superpowers/templates/CLAUDE-template.md`

- [ ] **步骤 1：create CLAUDE.md template**
...
| Verify | QA 关卡 | `bash scripts/qa.sh` |
...
### 任务 8： Quick Reference README

**涉及文件：**
- 新建：`docs/superpowers/templates/README.md`
...
> Requirement → brainstorm → spec → TaskMaster → tasks → clarify → plan → TDD(red→green) → QA 关卡 → done
...
### 任务 9： Verification —— Cross-Template Consistency Check

**涉及文件：**
- No new files. Verify existing templates.

- [ ] **步骤 1：check all template files exist**
...
- [ ] **步骤 2：check no AWS/Serverless references in any template**

Run: `grep -rin 'aws\|serverless\|cognito\|dynamodb\|vercel\|lambda\|cloudformation\|cloudwatch' docs/superpowers/templates/`
预期结果： 0 matches.

- [ ] **步骤 3：check no placeholder gaps**

Run: `grep -rn 'TBD\|TODO\|FIXME\|fill in later\|implement later' docs/superpowers/templates/`
预期结果： 0 matches (the `<!-- fill -->` markers in constitution are intentional and discoverable).

- [ ] **步骤 4：check file naming consistency**

Run: `grep -rn '\.spec\.md\|\.plan\.md\|tasks\.md' docs/superpowers/templates/`
预期结果： All references use the standardized extensions defined in the spec.

- [ ] **步骤 5：check agent role boundaries don't overlap**
...
- [ ] **步骤 6：commit verification results**
...
| 5 | `quality-gate.sh` | QA 关卡 script with failure-path guidance |
